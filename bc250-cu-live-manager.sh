...
select_asic() {
	local out value
	out="$("$UMR" -r "$ASIC.$REG_SPI" 2>&1 || true)"
	value="$(printf '%s\n' "$out" | parse_hex)"
	[ -n "$value" ] && return 0
...
try_read_reg_bank() {
	...
	while IFS= read -r candidate; do
		out="$("$UMR" -r "$ASIC.$candidate" -b "$se" "$sh" 0xffffffff 2>&1 || true)"
		value="$(printf '%s\n' "$out" | parse_hex)"
		if [ -z "$value" ]; then
			out="$("$UMR" -r "$ASIC.$candidate" -b "$se" "$sh" 2>&1 || true)"
			value="$(printf '%s\n' "$out" | parse_hex)"
		fi
		if [ -n "$value" ]; then
...
try_write_reg_global() {
	...
	while IFS= read -r candidate; do
		...
		if [ "$DRY_RUN" -eq 1 ]; then
			printf 'dry-run: %s -w %s %s\n' "$UMR" "$LAST_REG_PATH" "$value"
			return 0
		fi
		out="$("$UMR" -w "$LAST_REG_PATH" "$value" 2>&1 || true)"
	...
	while IFS= read -r candidate; do
		...
		if [ "$DRY_RUN" -eq 1 ]; then
			printf 'dry-run: %s -w %s %s -b %s %s 0xffffffff\n' \
				"$UMR" "$LAST_REG_PATH" "$value" "$se" "$sh"
			return 0
		fi
		out="$("$UMR" -w "$LAST_REG_PATH" "$value" -b "$se" "$sh" 0xffffffff 2>&1 || true)"
		if ! umr_output_failed "$out"; then
			return 0
		fi
	done < <(reg_candidates "$reg")
